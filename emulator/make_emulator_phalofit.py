...
from compute_preal_tables import compute_phalofit, ki
...
output_shape = (len(ki),) # output only a single P(ki)
...
for z in zs:
    ...
    for nn, iis in enumerate(zip(*Inds)):
        if nn%mpi_size == mpi_rank:
            ...
            P0gridii[iis] = compute_phalofit(coord,z=z)
